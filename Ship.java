...
public class Ship
{
    ...
    AnimationTimer calculateVelocityY = new AnimationTimer()
    {
        @Override
        public void handle(long now)
        {
            if (getAcceleration() == 17)
            {
                setVelocityY(getVelocityY() + (Math.sin(Math.toRadians(getRotation())) * (getAcceleration() * 0.01)));
            }
            else if (acceleration == -4)
            {
                setVelocityY(getVelocityY() + (Math.sin(Math.toRadians(getTempRotation())) * (getAcceleration() * 0.01)));
            }
        }
    };
    ...
    AnimationTimer shipAccelerationTimer = new AnimationTimer()
    {
        ...
        public void handle(long now)
        {
            ...
            wrapShip();
            if (getVelocity() == 0 && getVelocityX() == 0 && getVelocityY() == 0)
            {
    ...
    Ship()
    {
        ...
        getShipImage().setStroke(Color.WHITE);
